...
function setDeathsList(data: CovidCountrySummaryResponse[]) {
	const sorted = data.sort(
		(a: CovidCountrySummaryResponse, b: CovidCountrySummaryResponse) =>
			getUnixTimestamp(b.Date) - getUnixTimestamp(a.Date)
	);
	sorted.forEach((value: CovidCountrySummaryResponse) => {
...
function setRecoveredList(data: CovidCountrySummaryResponse[]) {
	const sorted = data.sort(
		(a: CovidCountrySummaryResponse, b: CovidCountrySummaryResponse) =>
			getUnixTimestamp(b.Date) - getUnixTimestamp(a.Date)
	);
...
function setChartData(data: CovidCountrySummaryResponse[]) {
	const chartData = data
		.slice(-14)
		.map((value: CovidCountrySummaryResponse) => value.Cases);
	const chartLabel = data
		.slice(-14)
		.map((value: CovidCountrySummaryResponse) =>
